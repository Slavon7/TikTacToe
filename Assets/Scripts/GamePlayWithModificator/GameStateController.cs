...
public class GameStateController : MonoBehaviour
{
    ...
    // Получаем доступ к компоненту Animator кнопки
    ...
    public void EndTurn()
    {
        ...
        else if (moveCount >= 48) GameOver("D");
        else
            ChangeTurn();
    }
    ...
    // Метод для добавления нажатой кнопки в список
    ...
    public void ChangeTurn()
    {
        ...
        if (turnCount == 7)
        {
            // После трех ходов удаляем первый ход игрока из возможных ходов
            Debug.Log("Удаляем первый ход");
            DisableButtonAnimation();
            // Очищаем текст на кнопке и делаем ее снова доступной для нажатия
            clickedButtons[0].GetComponentInChildren<Text>().text = "";
            clickedButtons[0].GetComponentInParent<TileController>().ResetTile();
            clickedButtons[0].GetComponentInParent<TileController>().SetAlpha(1f);
            clickedButtons[0].GetComponentInParent<Button>().interactable = true;
            clickedButtons.RemoveAt(0);
            turnCount = 6;
        }
        if (turnCount == 6)
        {
            // Проверяем, существует ли кнопка в списке и у нее есть ли компонент Animator
            if (clickedButtons.Count > 0 && clickedButtons[0] != null)
            {
                clickedButtons[0].GetComponentInChildren<Text>().text = "";
                buttonAnimator = clickedButtons[0].GetComponent<Animator>();

                // Включаем анимацию, если у кнопки есть компонент Animator
        ...
        // Это вызов тернарного оператора останется таким же
    ...
    public void DisableButtonAnimation()
    {
        ...
        if (clickedButtons.Count > 0 && clickedButtons[0] != null)
        {
            ...
            // Отключаем анимацию, если у кнопки есть компонент Animator
            if (buttonAnimator != null)
            {
        ...
        {
            Debug.LogError("Button not found or list is empty!");
        }
    }
    ...
    public void RestartGame()
    {
        ...
        {
            TileController tileController = tileList[i].GetComponentInParent<TileController>();
            tileController.ResetTile();

            // Выключаем анимацию, если у кнопки есть компонент Animator
            Animator buttonAnimator = tileController.interactiveButton.GetComponent<Animator>();
            if (buttonAnimator != null)
            {
                buttonAnimator.enabled = false;
            }
